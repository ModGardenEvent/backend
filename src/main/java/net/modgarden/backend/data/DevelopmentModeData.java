...
public class DevelopmentModeData {
	...
	public static void insertDevelopmentModeData() {
		try {
			long ultrusId;
			long calicoId;
			java.sql.PreparedStatement minecraftAccountStatement;
			try (Connection connection = ModGardenBackend.createDatabaseConnection()) {
				var userStatement = connection.prepareStatement(
						"INSERT OR IGNORE INTO users(id, username, display_name, discord_id, created, permissions, modrinth_id) VALUES (?, ?, ?, ?, ?, ?, ?)");
				ultrusId = RANDOM.nextLong(Long.MAX_VALUE);
				...
				calicoId = RANDOM.nextLong(Long.MAX_VALUE);
				...
				var eventStatement = connection.prepareStatement(
						"INSERT OR IGNORE INTO events(id, slug, display_name, discord_role_id, registration_time, start_time, end_time, freeze_time, minecraft_version, loader) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
				...
				var projectStatement = connection.prepareStatement(
						"INSERT OR IGNORE INTO projects(id, modrinth_id, attributed_to, slug) VALUES (?, ?, ?, ?)");
				...
				var projectAuthorsStatement = connection.prepareStatement(
						"INSERT OR IGNORE INTO project_authors(project_id, user_id) VALUES (?, ?)");
				...
				var awardsStatement = connection.prepareStatement(
						"INSERT OR IGNORE INTO awards(id, slug, display_name, sprite, discord_emote, tooltip, tier) VALUES (?, ?, ?, ?, ?, ?, ?)");
				...
				awardsStatement.setString(
						6,
						"You have collected %custom_data% out of 50 petals in the MoJank Fest event"
				);
				...
				var awardInstancesStatement = connection.prepareStatement(
						"INSERT OR IGNORE INTO award_instances(award_id, awarded_to, custom_data, submission_id, tier_override) VALUES (?, ?, ?, ?, ?)");
				...
				minecraftAccountStatement = connection.prepareStatement(
						"INSERT OR IGNORE INTO minecraft_accounts(uuid, user_id) VALUES (?, ?)");
			}
